    ...
    <DashboardLayout>
      <div className="p-6 md:p-8 max-w-7xl mx-auto font-['Inter',sans-serif]">
        <div className="mb-8">
          ...
          <p className="text-[var(--text-secondary)] mt-1 font-medium">Track your active ongoing service engagements.</p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 min-w-0">
            <div className="mb-6 flex items-center justify-between">
              <h3 className="text-xl font-bold text-[var(--text-primary)]">Ongoing Services</h3>
            </div>
            
            {services.length === 0 ? (
              <div className="bg-white rounded-2xl border border-[var(--border-color)] p-12 text-center">
                <i className="lnr lnr-hourglass text-5xl text-[var(--text-secondary)] mb-4 block"></i>
                <h4 className="text-lg font-bold text-[var(--text-primary)] mb-1">No Ongoing Services</h4>
                <p className="text-sm text-[var(--text-secondary)]">You don't have any active service engagements yet.</p>
              </div>
            ) : (
            <div className="space-y-4">
              {services.map((service: any) => (
                <div key={service.id} className="bg-white rounded-2xl border border-[var(--border-color)] p-6 hover:border-[var(--accent-color)] transition-all flex flex-col md:flex-row gap-6 relative group">
                  {/* Premium Badge */}
                  {service.isPremium && (
                  ...
                  {/* Avatar */}
                  <div className="w-16 h-16 rounded-2xl bg-[var(--accent-color)] shrink-0 flex items-center justify-center text-white font-black text-2xl mt-2 md:mt-0">
                  ...
                  <div className="flex-1">
                    ...
                    <h4 className="text-xl font-bold text-[var(--text-primary)] hover:text-[var(--accent-color)] transition-colors cursor-pointer mb-4 leading-tight pr-12 md:pr-0">
                      {service.title}
                    </h4>
                  ...
                  <div className="md:text-right flex flex-row md:flex-col items-center md:items-end justify-between gap-4 border-t md:border-t-0 md:border-l border-[var(--border-color)] pt-4 md:pt-0 md:pl-6 min-w-[160px]">
                    ...
                    <div className="flex items-center gap-3 bg-emerald-50 px-3 py-2 rounded-xl border border-emerald-100 w-full md:w-auto">
                      ...
                      <div className="text-left">
                        ...
                        <div className="text-xs font-bold text-[var(--text-primary)]">{service.hiredName}</div>
                      </div>
                    </div>
                    )}
                    
                    <Link to={`/dashboard/ongoing-single/${service.id}`} className="w-full md:w-auto px-6 py-2.5 rounded-xl bg-[var(--accent-color)] hover:bg-[var(--accent-hover)] text-white font-bold text-sm text-center shadow-lg transition-all active:scale-95 whitespace-nowrap mt-auto">
                      View Details
                    </Link>
                  </div>
                </div>
