    ...
    <DashboardLayout>
      <div className="p-6 md:p-8 max-w-7xl mx-auto font-['Inter',sans-serif]">
        <div className="mb-8">
          ...
          <p className="text-[var(--text-secondary)] mt-1 font-medium">Review your history of successfully completed service engagements.</p>
        </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {services.length === 0 ? (
              <div className="bg-white rounded-2xl border border-[var(--border-color)] p-12 text-center">
                <i className="lnr lnr-checkmark-circle text-5xl text-[var(--text-secondary)] mb-4 block"></i>
                <h4 className="text-lg font-bold text-[var(--text-primary)] mb-1">No Completed Services</h4>
                <p className="text-sm text-[var(--text-secondary)]">You haven't completed any services yet.</p>
              </div>
            ) : (
            <div className="space-y-4">
              {services.map((service: any) => (
                <div key={service.id} className="bg-white rounded-2xl border border-[var(--border-color)] p-6 hover:border-emerald-200 transition-all flex flex-col md:flex-row gap-6 relative group overflow-hidden">
                  ...
                  <div className="flex-1">
                    ...
                    <h4 className="text-xl font-bold text-[var(--text-primary)] hover:text-emerald-600 transition-colors cursor-pointer mb-4 leading-tight pr-12 md:pr-0">
                      {service.title}
                    </h4>
                  ...
                  <div className="md:text-right flex flex-row md:flex-col items-center md:items-end justify-between gap-4 border-t md:border-t-0 md:border-l border-[var(--border-color)] pt-4 md:pt-0 md:pl-6 min-w-[160px]">
                    ...
                    <Link to={`/dashboard/ongoing-single/${service.id}`} className="w-full md:w-auto px-6 py-2.5 rounded-xl bg-[var(--bg-secondary)] text-[var(--text-primary)] font-bold text-sm text-center hover:bg-gray-200 transition-colors active:scale-95 whitespace-nowrap mt-auto">
                      View Details
                    </Link>
                  </div>
                </div>
