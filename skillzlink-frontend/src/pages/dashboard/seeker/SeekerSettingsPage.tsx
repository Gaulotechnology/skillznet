...
import { getCurrentUser, logout, seekerApi } from "../../../services/api";
...
export function SeekerSettingsPage() {
  ...
  useEffect(() => {
    seekerApi.getSettings().then((data: any) => {
  ...
  const handleSaveNotifications = async () => {
    setSaving(true);
    try {
      await seekerApi.updateSettings(notifications);
  ...
  const handlePasswordReset = async () => {
    try {
      await seekerApi.requestPasswordReset();
