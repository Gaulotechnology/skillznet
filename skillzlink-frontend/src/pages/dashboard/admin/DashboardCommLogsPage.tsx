...
export function DashboardCommLogsPage() {
  ...
  const channelColor: Record<string, string> = { "In-App": "bg-[var(--accent-light)] text-blue-700", SMS: "bg-[var(--accent-light)] text-[var(--accent-color)]", Email: "bg-[var(--bg-secondary)] text-[var(--text-primary)]" };
  const statusColor: Record<string, string> = { Read: "bg-green-50 text-green-700", Unread: "bg-yellow-50 text-yellow-700", Delivered: "bg-[var(--bg-secondary)] text-[var(--text-secondary)]" };
  ...
  const columns: Column<any>[] = [
    ...
    { key: "subject", label: "Preview", render: (row) => <span className="text-[var(--text-secondary)] text-xs truncate max-w-[200px] block">{row.subject.length > 45 ? row.subject.slice(0, 45) + "…" : row.subject}</span> },
    ...
    <DashboardLayout>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white border border-[var(--border-color)] rounded-lg p-4">
          ...
          <p className="text-2xl font-semibold text-[var(--text-primary)] mt-1">{today}</p>
        </div>
        <div className="bg-white border border-[var(--border-color)] rounded-lg p-4">
