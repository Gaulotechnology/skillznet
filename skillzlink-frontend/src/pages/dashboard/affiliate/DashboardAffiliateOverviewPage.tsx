    ...
    <DashboardLayout>
      <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-slate-800">Affiliate Overview</h2>
            <p className="text-slate-500 mt-1">Manage your referral links and track your commissions.</p>
          </div>
          <button onClick={handleRequestPayout} disabled={requestingPayout} className="px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl transition-colors shadow-sm active:scale-95">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            ...
            <h3 className="text-slate-500 font-medium mb-1">Total Clicks</h3>
            <div className="text-3xl font-black text-slate-800">{stats.total_clicks}</div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            ...
            <h3 className="text-slate-500 font-medium mb-1">Total Signups</h3>
            <div className="text-3xl font-black text-slate-800">{stats.total_signups}</div>
          </div>

          <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            <div className="w-12 h-12 rounded-2xl bg-indigo-50 text-indigo-500 flex items-center justify-center text-xl mb-4">
              <i className="lnr lnr-diamond"></i>
            </div>
            <h3 className="text-slate-500 font-medium mb-1">Total Earnings</h3>
            <div className="text-3xl font-black text-slate-800">${stats.total_earnings.toFixed(2)}</div>
          </div>
        </div>

        <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
          <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
            <i className="lnr lnr-link text-rose-500"></i> Your Referral Link
          </h3>
          <div className="flex items-center gap-4">
            <input 
              ...
              value={referralLink}
            />
            <button onClick={handleCopy} className="px-6 py-3 bg-rose-500 hover:bg-rose-600 text-white font-bold rounded-xl transition-colors shadow-sm shadow-rose-200">
        ...
        {recentReferrals.length > 0 && (
          <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
            <div className="p-6 border-b border-slate-100">
              <h3 className="font-bold text-lg text-slate-800">Recent Referrals</h3>
            </div>
            <div className="divide-y divide-slate-100">
              {recentReferrals.map((ref: any, i: number) => (
                <div key={i} className="p-5 flex items-center justify-between hover:bg-slate-50 transition-colors">
                  <div>
                    <div className="font-bold text-slate-800 text-sm">{ref.name || ref.email || "User"}</div>
                    <div className="text-xs text-slate-500 font-medium">{ref.date || ref.created_at || ""}</div>
                  </div>
