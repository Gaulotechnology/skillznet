	                 	...
	                 	$Result['pagelayout'] = 'login';
                   	    ...
                   	    if (isset($Params['module']['ajax']) && $Params['module']['ajax'] == true){
                            header('X-Robots-Tag: noindex');
                               ...
                               $pos = strrpos($url->buildUrl(), self::$currentModuleName.'/'.self::$currentView);
                               if ($pos === false) {
                                   $rPath = self::$currentModuleName . '/' . self::$currentView;
                               } else {
                                   $rPath = substr($url->buildUrl(), $pos);
                               }
                               $r = '/(r)/'.rawurlencode(base64_encode($rPath));
